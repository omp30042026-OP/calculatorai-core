import { createDecisionV2, applyDecisionEvent } from "../packages/decision/src/engine.js";

function run() {
  let d = createDecisionV2({
    decision_id: "dec_1",
    meta: { title: "Promo Plan", owner_id: "user_1" },
    artifacts: {},
  });

  const events = [
    { type: "VALIDATE", actor_id: "user_1" } as const,
    { type: "SIMULATE", actor_id: "user_1", simulation_snapshot_id: "snap_123" } as const,
    { type: "EXPLAIN", actor_id: "user_1", explain_tree_id: "tree_456" } as const,
    { type: "APPROVE", actor_id: "user_1" } as const,
  ];

  for (const e of events) {
    const r = applyDecisionEvent(d, e);
    if (!r.ok) throw new Error(JSON.stringify(r, null, 2));
    d = r.decision;
  }

  console.log("✅ final decision:", JSON.stringify(d, null, 2));
}

run();

